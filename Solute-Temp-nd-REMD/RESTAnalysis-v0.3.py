...
def helicity_per_residus(dssp,n_frames ):
    ...
    for j in range(len(dssp[0])):
            ...
            lengthtraj= n_frames
            Hy.append(dictss.get('H',0)/(lengthtraj)*100)
            Ey.append(dictss.get('E',0)/(lengthtraj)*100)
...
def runCluster(topolPrefix , trajPrefix, rep ):
        ...
        t = md.load( str(rep) + '.striped_traj.nc' , top='system.wat.leap.strip.prmtop' )
        ...
        #for i in range(len(metric2)): metric2[i] = np.reshape(metric[i] , (5, 2), order='F')
            ...
            filename='cluster%s_%s.nc' %(rep,nb_cluster-1)
            filenamepdb='cluster%s_%s.pdb' %(rep,nb_cluster-1)
...
def find_correlation(nb_cluster, rep=1):
    ...
    for i in range(len(nb_cluster)):
        ...
        print(CatoCA)
        plt.subplot2grid( (len(nb_cluster) , 4), (i,2)  )
        plt.ylim(0.55,0.85)
        plt.plot(CatoCA ,label = dssp[-1])
        plt.subplot2grid( (len(nb_cluster) , 4), (i,3)  )
        helicity_per_residus(dssp,traj.n_frames)
        for h in range(2) :
            plt.subplot2grid( (len(nb_cluster) , 4), (i,h)  )
...
def diffusionAnalysis(nreps,logfile,topolPrefix, trajPrefix,relevantcluster ):
    f = open('rem.log','r')
    started =0
    out=[0]
    prev=[0]
...
def main() :
 ...
 if  cluster == True or corr == True :
    for i in range(nreps) :
        c, nb_cluster = runCluster(  topolPrefix, trajPrefix , i )
 ...
 if  exch ==True :
     diffusionAnalysis(nreps,logfile, topolPrefix, trajPrefix, relevantcluster   )
