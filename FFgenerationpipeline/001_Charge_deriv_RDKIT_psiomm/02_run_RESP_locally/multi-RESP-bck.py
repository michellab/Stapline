...
def run_allconf(n_conf, n_cpu=10) :
    ...
    for i in range(1,n_conf+1):
      args.append('%s' %(i))
      os.system('antechamber -i conf%s.pdb -fi pdb -o %s.gau  -fo gcrt -gv 2 -ge %s.gesp'  %(i, i, i))
      #run_resp('%s' %(i))
    ...
    for input in range(1,n_conf+1):
        os.system('antechamber -i  %s.log -fi gout -o %s.mol2 -fo mol2 -c resp -at gaff2   -eq 2  -pf   -dr n' %(input,input) )
...
def multiple(n_conf,resname='MOL'):
    ...
    makeamberlibfile(index, parm )

def makeamberlibfile(frag, parm , resname='MOL'):
    tleapinput=open('tleap.in' ,'w')
    tleapinput.write('%s = loadmol2 newcharges.mol2 \n' %(resname) )
    for atom in  parm.atoms:
        if atom.idx not in frag :
    ...
    tleapinput.write('impose MOL { 1 2 3 } { { "N" "CA" "C" "O" 0.0 } { "H" "N" "CA" "C"  0.0 } { "CA" "CB" "CC" "CD" 180 } { "CB" "CC" "CD" "CE" 180 } } \n')
...
parser.add_argument('--last',type=int, nargs='*', help='last atom in side chain to take in account for partial charge calculation, default:none ')
